...
@Component({
  ...
  templateUrl: './opdifference.component.html',
  styleUrls: ['./opdifference.component.css']
})
export class OpdifferenceComponent implements OnInit {
  ...
  getDefaultOpName() {
    ...
    let url = '/echart/operatormonitor/getOpt_OperatorDefectMDLRENYUANBIEdefaultopName';
  ...
  getProductCategory() {
    ...
    let url = '/echart/operatormonitor/getOpt_OperatorDefectCategory';
  ...
  drawChart() {
    ...
    let url = '/echart/operatormonitor/getOpt_OperatorDefectMDLRENYUANBIE';

    this.httpService.post(url, this.chartParam).subscribe(
      (res) => {
        this.respText = res;
        this.echartOption = this.echartService.getOption_OperatorDefectrenyuanbie(this.processType + ' 良率指标', this.respText, this.waterMark, this.deviceType);
      },
      (error) => {
        console.log(error);
      }
  ...
  drawChart1() {
    ...
    let url = '/echart/operatormonitor/getOpt_OperatorDefectMDLRENYUANBIEOne';

    this.httpService.post(url, this.chartParam).subscribe(
      (res) => {
        this.respText = res;
        console.log('this.waterMark:' + this.waterMark);
        this.echartOption1 = this.echartService.getOption_OperatorDefectrenyuanbieOne(this.defaultOpName + this.processType + ' 产能监控', this.respText, this.waterMark, this.deviceType);
      },
      (error) => {
        console.log(error);
      }
