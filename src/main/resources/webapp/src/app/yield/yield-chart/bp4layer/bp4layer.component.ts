...
@Component({
  ...
  templateUrl: './bp4layer.component.html',
  styleUrls: ['./../../../production/pro-eac/pro-eac.component.css']
})
export class Bp4layerComponent implements OnInit {
  ...
  getProductCategory() {
    ...
    this.httpService.get('/echart/yield/yield4LayerProductCategory', options).subscribe(
      (res) => {
  ...
  drawChart() {
    ...
    this.httpService.post('/echart/yield/yield4Layer', this.chartParam).subscribe(
      (res) => {
        this.respText = res;
