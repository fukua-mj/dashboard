...
@Component({
  ...
  templateUrl: './warehouse-time.component.html',
  styleUrls: ['../warehouse-grade/warehouse-grade.component.css']
})
export class WarehouseTimeComponent implements OnInit {
  ...
  drawChart() {
    ...
    this.httpService.post(url, this.chartParam).subscribe(
      (res) => {
        this.respText = res;
        this.echartOption = this.echartService.getOption_warehouseTime('库龄信息', this.respText, this.waterMark, this.deviceType);
      },
      (error) => {
        console.log(error);
      }
